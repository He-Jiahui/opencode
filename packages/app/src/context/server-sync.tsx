...
export const loadWorkflowsQuery = (scope: ServerScope, directory: string, sdk: OpencodeClient) =>
  queryOptions({
    queryKey: [scope, directory, "workflow"] as const,
    queryFn: () => sdk.workflow.list().then((r) => r.data ?? []),
  })

export const loadWorkflowGraphQuery = (scope: ServerScope, directory: string, workflowID: string, sdk: OpencodeClient) =>
  queryOptions({
    queryKey: [scope, directory, "workflow-graph", workflowID] as const,
...
function makeQueryOptionsApi(
  ...
  return {
    ...
    workflows: (directory: PathKey) => loadWorkflowsQuery(scope, directory, sdkFor(directory)),
    workflowGraph: (directory: PathKey, workflowID: string) =>
      loadWorkflowGraphQuery(scope, directory, workflowID, sdkFor(directory)),
  }
}
