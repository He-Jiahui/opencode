...
export default function LegacyLayout(props: ParentProps) {
  ...
  async function renameSession(session: Session, title: string) {
    ...
    const result = await serverSDK()
      .client.session.update({ directory: session.directory, sessionID: session.id, title: next })
    ...
    const [, setChildStore] = serverSync().child(session.directory)
    setChildStore(
      produce((draft) => {
        const match = Binary.search(draft.session, session.id, (item) => item.id)
        if (match.found) draft.session[match.index] = result
      }),
    )
  }

  function togglePinSession(session: Session) {
    const key = `${pathKey(session.directory)}:${session.id}`
    setStore("pinnedSession", key, !(store.pinnedSession?.[key] === true))
  }
  ...
  async function exportSession(session: Session) {
    ...
    const messages = [...(childStore.message[session.id] ?? [])].sort(
    ...
    anchor.href = url
    anchor.download = `${sessionExportFilename(session.title || session.id)}.md`
    anchor.click()
...
function sessionExportFilename(value: string) {
  const cleaned = value.replace(/[<>:"/\\|?*\x00-\x1f]/g, " ").trim().replace(/\s+/g, "-")
